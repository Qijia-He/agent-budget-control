#!/bin/bash
#SBATCH --job-name=router_3cls_v6_v4meta_qwen3_4b_full
...
# This run: Qwen3-4B FULL fine-tune.
...
#
# Uses DeepSpeed ZeRO-2 (examples/deepspeed/ds_z2_config.json, ships with
# LLaMA-Factory) to shard optimizer states + gradients across the 4 GPUs --
# full fine-tuning a 4B model's Adam optimizer state does not fit on a single
# GPU otherwise. deepspeed must be installed in the conda env (already done
# for the 4cls full-FT run -- if this errors with ModuleNotFoundError:
# deepspeed, `pip install deepspeed` into the env below).
...
FORCE_TORCHRUN=1 llamafactory-cli train /path/to/agent-budget-control/sft_runs/qwen3_4b_full.yaml
